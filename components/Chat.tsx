...
const Chat: React.FC<ChatProps> = ({ data }) => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="h-[400px] border border-border rounded-md p-4 flex flex-col space-y-4 overflow-y-auto bg-muted/50">
          {messages.length === 0 && (
            <div className="text-center text-muted-foreground m-auto">
                <p>No messages yet. Ask a question to start!</p>
                 <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-2">
                    {exampleQueries.map((q, i) => (
                        <button key={i} onClick={() => handleExampleQueryClick(q)} className="p-2 border border-border rounded-md text-sm hover:bg-accent hover:text-accent-foreground transition-colors text-left">
                           {q}
                        </button>
                    ))}
                </div>
          ...
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-xs md:max-w-md lg:max-w-2xl p-3 rounded-lg ${msg.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-card text-card-foreground'}`}>
                 {msg.sender === 'bot' ? (
               ...
               {msg.sender === 'user' && <span className="flex-shrink-0 flex items-center justify-center h-8 w-8 rounded-full bg-secondary text-secondary-foreground"><UserIcon className="h-5 w-5" /></span>}
            </div>
          ))}
          {isLoading && (
             <div className="flex items-start gap-3 justify-start">
               <span className="flex-shrink-0 flex items-center justify-center h-8 w-8 rounded-full bg-primary text-primary-foreground"><BotIcon className="h-5 w-5" /></span>
                <div className="max-w-xs md:max-w-md lg:max-w-lg p-3 rounded-lg bg-card">
